...
import isDef from '@shared/isDef';
import emptyFn from '@shared/emptyFn';
import DndElement from '../DndElement';
import { DndContextSymbol, DndParentElementSymbol } from './Symbols';

import type { ShallowRef } from 'vue';
import type { DndInstance, DndEventTable } from './UseDndInstance';
import type {
  DndOptions,
  DndClasses,
  InternalDndOptions,
  ...
  BaseDragEndEvent,
  BaseDropEvent,
  BaseDragPreventEvent,
  MirrorOptions,
} from '../types';
import type { AbstractDndOptions as BaseAbstractDndOptions } from '@base/AbstractDnd';
...
export const defaultMirrorOptions: MirrorOptions = {
  ...
  appendTo: ({ source }) => source.nativeEl!.parentElement || document.body,
};
...
function useDndElementMap() {
  ...
  const getDndElementByNativeEl = markRaw(
    (nativeEl: Element) => map.get(nativeEl) || null
  );
  const storeDndElement = markRaw((nativeEl: Element, dndEl: DndElement) => {
    map.set(nativeEl, dndEl);
  });
  const deleteDndElementByNativeEl = markRaw((nativeEl: Element) => {
    map.delete(nativeEl);
  });
  return {
    storeDndElement,
...
export function useContextInjector<O extends DndOptions>(
  ...
  onMounted(() => {
    ...
    if (instance.rootDndElement.nativeEl) {
      markRaw(instance.rootDndElement.nativeEl);
    }
  });
  ...
  const isDraggable = markRaw((source: Element) => {
    const internalEl = getDndElementByNativeEl(source);
    return internalEl?.draggable ?? false;
  });
  const isDroppable = markRaw((dropzone: Element) => {
  ...
  const context = reactive({
    options,
    instance: shallowRef(instance),
    storeDndElement,
    getDndElementByNativeEl,
    deleteDndElementByNativeEl,
    classnamesOf: markRaw((id) => {
      if (id === 'source' || id === 'dropzone' || id === 'handle') {
        return ensureArray(toValue(options)[id]);
      }
      return ensureArray(toValue(options).classes[id]);
    }),
    isDraggable,
    isDroppable,
  }) as DndContext<O>;
...
export function useBaseDnd<O extends BaseAbstractDndOptions>(
  constructBaseDnd: (options: O) => BaseAbstractDnd,
  options: () => O
) {
  const baseDnd = shallowRef<BaseAbstractDnd | null>(null);
...
export function useBaseDndBridge(
  context: DndContext,
  baseDnd: ShallowRef<BaseAbstractDnd | null>
) {
  ...
  const connectBaseDnd = (dnd: BaseAbstractDnd) => {
    dnd
      .on('drag', onDrag)
